...
export const BIOME_GROUND_MATERIALS: Record<Biome, MaterialKey> = {
  ...
  tropical: 'ground.tropical',
  swamp: 'ground.swamp',
  wetland: 'ground.wetland',
  'rocky-highlands': 'ground.rocky-highlands',
  savanna: 'ground.savanna',
  badlands: 'ground.badlands',
  'bamboo-forest': 'ground.bamboo-forest',
};

const BIOME_ORDER: Biome[] = [
  'meadow', 'forest', 'dunes', 'scrapflats', 'tropical',
...
export function biomeWeightsAt(x: number, z: number): Record<Biome, number> {
  ...
  const warmth = fbm(warped.x / 190 - 57.4, warped.z / 190 + 83.2, 7823, 2);
  const character = fbm(warped.x / 118 + 17.1, warped.z / 118 - 29.6, 9419, 2);
  ...
  // biomes read as arrivals *from*. Tropical is low on purpose — it should
  // read as somewhere you *arrive at*, the plan's own words — and it sits in
  // the wettest corner of the moisture field, past forest, with forest's
  // high-ground preference flipped (see `docs/tropical-biome-plan.md`).
  // Getting there took three rounds recorded here for the next tuner:
  // tropical at moisture 0.74 sampled 1.2% (the wet tail is thin), and
  // without forest's widened roughness tolerance forest ceded the whole wet
  // end and sampled 7.7% — the redwood biome must not starve.
  const scores: Record<Biome, number> = {
    // Damp and mid-height: the gentle country that joins everything else.
    meadow: 0.85 - Math.abs(moisture - 0.16) * 2.0 - Math.abs(height - 0.45) * 0.65,
    // Damp and rough, favouring higher ground. Peak raised when tropical
    // arrived: the wet end of the moisture field is now split by altitude,
    // and without this forest ceded all of it (it sampled 7.7% before the
    // fix — redwoods live here, so it must not starve).
    forest: 1.03 - Math.abs(moisture - 0.48) * 1.7 - Math.abs(roughness - 0.32) * 0.95 + height * 0.45,
    // Dry, smooth, and low — sand collects in the basins.
    dunes: 0.73 - Math.abs(moisture + 0.5) * 1.75 - Math.abs(roughness + 0.18) * 1.05 + (1 - height) * 0.45,
    // Dry and rough.
    scrapflats: 0.815 - Math.abs(moisture + 0.1) * 1.5 - Math.abs(roughness - 0.5) * 1.15,
    // The wettest ground of all, favouring low-to-mid heights, mid
    // roughness — forest's corner with the height preference flipped, so the
    // two split the wet end of the moisture field by altitude instead of
    // tropical quietly starving the redwoods.
    tropical: 0.985 - Math.abs(moisture - 0.62) * 1.18 - Math.abs(roughness - 0.3) * 0.85 + (1 - height) * 0.55,
    swamp: 1.02 - Math.abs(moisture - 0.72) * 1.35 - Math.abs(height - 0.18) * 1.15 - Math.abs(character + 0.35) * 0.55,
    wetland: 1.0 - Math.abs(moisture - 0.52) * 1.45 - Math.abs(height - 0.28) * 1.0 - Math.abs(character - 0.42) * 0.55,
    'rocky-highlands': 1.04 - Math.abs(roughness - 0.65) * 1.15 - Math.abs(height - 0.86) * 1.25 - Math.abs(warmth + 0.15) * 0.45,
    savanna: 0.96 - Math.abs(moisture + 0.24) * 1.3 - Math.abs(warmth - 0.55) * 0.8 - Math.abs(height - 0.48) * 0.55,
    badlands: 1.0 - Math.abs(moisture + 0.55) * 1.2 - Math.abs(roughness - 0.5) * 0.9 - Math.abs(character - 0.2) * 0.4,
    'bamboo-forest': 1.02 - Math.abs(moisture - 0.58) * 1.15 - Math.abs(warmth - 0.2) * 0.65 - Math.abs(character - 0.62) * 0.8,
  ...
  const weights: Record<Biome, number> = {
    meadow: 0, forest: 0, dunes: 0, scrapflats: 0, tropical: 0,
    swamp: 0, wetland: 0, 'rocky-highlands': 0, savanna: 0, badlands: 0, 'bamboo-forest': 0,
    clearing: 0,
  };
  let total = 0;
