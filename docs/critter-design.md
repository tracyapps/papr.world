...
Parrot (added 2026-09-18): the tropical biome's flagship — the tropical-biome plan's own cheapest-right answer, "reuses the existing bird rig with new colours and a beak." Three silhouette cues separate it from the songbird at a glance: a stout hooked beak (a fat upper cone curving down over a small dark lower one), a long two-panel tail that drags and swings behind it, and slow broad wingbeats (half the songbird's flap rate, more travel) — a parrot flies like it means it. Coats lean on the jungle's own papers (tropical frond, banana leaf, flower scatter, vine lattice) plus a plain bright green and one blue, because every flock has one. Its idle behavior is sideways where the bird's is downward: instead of pecking, the head swings back toward the wing in a preening arc, heavy on look-around and groom. Its flourish is a big wing stretch with the tail lifting and an indignant little shimmy. Its curious tell is the same head-cock the songbird has — a parrot just commits to it. Voice note for authored dialogue: parrots repeat things, loudly, with better timing; greetings and self-replies live in `conversations.json` with every other species.

Toucan, sloth, monkey (added 2026-09-18): the jungle's **canopy crowd** — the first critters that live *in* the trees rather than under them. Movement lives in `src/game/critterCanopy.ts`; see "Canopy Behavior" below.
- **Toucan** — the parrot's skeleton with the one cue that matters: an enormous tall, narrow bill, plus a black body, yellow bib, and blue eye ring. Coat convention is inverted for this species only: the body is always dark, and the coat's paper/accent goes on the *bill* (a keel-billed toucan's rainbow bill is the whole point). Idle is a slow side-to-side bill swing; flourish is the berry toss (a berry arcs up out of the bill and is caught again). Flies perch to perch in burst-and-glide flaps, hops on the ground for short visits.
- **Sloth** — round and shaggy, long hooked limbs, the pale face mask with dark eye stripes and a small permanent smile. Built as a low quadruped like everyone else; hanging is the canopy layer rolling the whole animal upside down, which puts all four hooked feet on the branch, and the neck turns the face back upright (the upside-down sloth gaze). Everything is slow: speed 0.24, idle actions run 2.6x slower, a trip down a trunk takes about half a minute. Always has the `sleepy` trait. Algae-green moss coat is the most accurate one. Flourish: a very slow wave.
- **Monkey** — big round ears, pale face/belly, long arms, a curly tube tail. Sits on branches with legs dangling, swings hand over hand, leaps between trees, knuckle-walks on the ground. Always `mischievous`. Most likely of the three to come down and see you by itself. Flourish: a backflip (the rig spins around its middle via a `spin` group).

## Canopy Behavior (2026-09-18)

`critterCanopy.ts` gives tree-dwellers somewhere to be and ways to get there:
...
- **Talking up into the tree.** `critterReachDistance` uses ground distance while a critter is aloft, so you can talk to a sloth hanging overhead. Petting still needs it within arm's reach.
